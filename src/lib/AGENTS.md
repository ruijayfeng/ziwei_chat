...
agent/chat-runtime.ts: Runtime tool stores, persistence selection, and profile data deletion for the MVP API route.
agent/chat-persistence.ts: Storage-agnostic chat message, tool event, and profile deletion persistence contract with in-memory implementation.
...
db/chat-persistence.ts: Postgres-backed ChatPersistence adapter for messages and tool events.
