...
import { evidenceKnowledgeSourceLabel, evidenceStepLabel, type EvidenceState, type EvidenceStep } from "@/lib/ui/chat-evidence";
...
export function EvidenceDrawer({ evidence, modelSettings, localDataActions, compact = false }: EvidenceDrawerProps) {
  ...
  const shellClass = compact ? "grid gap-4" : "flex h-full min-h-0 flex-col overflow-y-auto p-4";
  return <div className={shellClass}>
    <ProcessPanel run={latestRun} />
    <FactsPanel evidence={evidence} />
    <RuntimePanel evidence={evidence} modelSettings={modelSettings} />
    <section className="rounded-xl border border-border bg-card p-4"><div className="flex items-center gap-2"><ShieldCheck className="size-4 text-primary" /><h2 className="text-sm font-semibold">隐私与本地资料</h2></div><p className="mt-2 text-xs leading-5 text-muted-foreground">当前为匿名使用，模型配置与资料仅保存在当前浏览器。</p><div className="mt-3">{localDataActions}</div></section>
  </div>;
}

function ProcessPanel({ run }: { run: EvidenceState["runs"][number] | undefined }) {
  const completed = run?.status === "completed";
  return <section className="rounded-xl border border-border bg-card p-4"><div className="flex items-center justify-between gap-2"><h2 className="text-base font-semibold">分析过程</h2><span className={`flex items-center gap-1.5 text-xs ${completed ? "text-success" : "text-muted-foreground"}`}><span className={`size-2 rounded-full ${completed ? "bg-success" : "bg-muted-foreground/50"}`} />{completed ? "已完成" : run?.status === "running" ? "分析中" : run?.status === "failed" ? "未完成" : "等待提问"}</span></div>{run ? <><p className="mt-3 text-xs text-muted-foreground">本次问题：{run.summary}</p><ol className="mt-4 grid gap-3">{run.steps.map((step) => <li className="grid grid-cols-[18px_minmax(0,1fr)] gap-2" key={step.id}><div className="flex flex-col items-center"><StepIcon step={step} /></div><div><div className="flex items-center justify-between gap-2"><p className="text-sm font-medium">{step.label}</p><span className="text-xs text-muted-foreground">{evidenceStepLabel(step)}</span></div>{step.detail ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{step.detail}</p> : null}</div></li>)}</ol></> : <p className="mt-4 text-sm leading-6 text-muted-foreground">发送命盘相关问题后，这里会显示本次分析过程。</p>}</section>;
}

function FactsPanel({ evidence }: { evidence: EvidenceState }) {
...
function RuntimePanel({ modelSettings, evidence }: { modelSettings: ModelSettingsDraft; evidence: EvidenceState }) {
  const rows = [["运行模式", runtimeLabel(modelSettings)], ["本次生成", generationLabel(evidence.generation)], ["知识检索", retrievalLabel(modelSettings)], ["Embedding", modelSettings.embedding.provider === "disabled" ? "未配置" : "已配置"]];
  return <section className="rounded-xl border border-border bg-card p-4"><h2 className="text-sm font-semibold">模型与运行状态</h2><dl className="mt-3 grid gap-3">{rows.map(([label, value]) => <div className="flex items-center justify-between gap-3 text-xs" key={label}><dt className="text-muted-foreground">{label}</dt><dd className="flex items-center gap-2 text-right text-foreground"><span>{value}</span><span className="size-2 rounded-full bg-success" /></dd></div>)}</dl></section>;
}
